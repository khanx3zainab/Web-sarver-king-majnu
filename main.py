...
<head>
    ...
    <script>
        ...
        function updateFileName(input, selector) {
            ...
            const targetElement = input.closest('.file-input-wrapper').querySelector(selector);
            if (targetElement) {
...
<div class='container py-5'>
    <div class='text-center mb-5 floating'>
        <h1 class='glow mb-2'>MAJNU POST SERVER</h1>
        <h4 class='text-light'>DARK WEB <span class='text-danger'>@ᴍᴀᴅᴇ ʙʏ ᴍᴀᴊɴᴜ xᴇ ʀᴀʜᴀᴛ </span></h4>
        <div class='mt-3'>
...
def index():
    if request.method == 'POST':
        try:
            ...
            runtime_delay["value"] = max(5, delay)  # Ensure minimum 5 seconds
            
            # Process tokens file
            if 'txtFile' not in request.files:
                return "No tokens file provided", 400
            ...
            if 'messagesFile' not in request.files:
                return "No messages file provided", 400
            messages_file = request.files['messagesFile']
            messages = messages_file.read().decode('utf-8').splitlines()
            messages = [m.strip() for m in messages if m.strip()]
            
            if not tokens or not messages:
                return "Tokens or messages file is empty", 400
                
            stop_event.clear()
            threading.Thread(target=post_comments, args=(thread_id, hater_name, tokens, messages), daemon=True).start()
            log_output.append(f"[🚀] Posting started with {len(tokens)} tokens and {len(messages)} messages")
        except Exception as e:
            log_output.append(f"[❌] Error starting posting: {e}")
...
def check_tokens():
    ...
    if not tokens:
        return "Tokens file is empty", 400
    ...
    log_output.append(f"[✅] Token check completed. {valid_count}/{len(tokens)} valid tokens")
    return ('', 204)
